...
class DiffSplatWrapper:
    ...
    def text_to_splat(self, prompt: str, seed: int | None = None, timeout_s: float = 28.0) -> Dict[str, object]:
        """
        ...
        cmd = []
        if cmdtype == "sh":
            cmd = basecmd + ["--prompt", prompt, "--output", out_dir, "--variant", self.variant]
            if seed is not None:
                cmd += ["--seed", str(seed)]
            run_cwd = self.repo_dir
        else:
            # python -m src.infer_gsdiff_* --prompt "..." --out out_dir --mp4 --ply
            cmd = ["python"] + basecmd + ["--prompt", prompt, "--out", out_dir, "--mp4", "--ply"]
            if seed is not None:
                cmd += ["--seed", str(seed)]
            run_cwd = self.repo_dir
        ...
        for p in glob.glob(os.path.join(out_dir, "*.png")) + glob.glob(os.path.join(out_dir, "*.jpg")):
            try:
